...
namespace xxx
{
    AMaterialTemplate::AMaterialTemplate() :
        AMaterial(Type::MaterialTemplate),
    ...
    void AMaterialTemplate::serialize(Json& json, std::vector<char>& binary, std::vector<std::string>& reference) const
    {
    ...
    void AMaterialTemplate::deserialize(const Json& json, const std::vector<char>& binary, const std::vector<std::string>& reference)
    {
        ...
        for (Json::const_iterator it = parametersJson.begin(); it != parametersJson.end(); it++)
        {
            ...
            else if (it.value().is_string())
            {
                ...
                ATexture* texture = AssetManager::loadAsset<ATexture>(reference[index]);
    ...
    void AMaterialTemplate::setShadingModel(ShadingModel shadingModel)
    {
        _shadingModel = shadingModel;
        _stateSetDirty = true;
    }

    void AMaterialTemplate::setAlphaMode(AlphaMode alphaMode)
    {
        _alphaMode = alphaMode;
        _stateSetDirty = true;
    }

    void AMaterialTemplate::setDoubleSided(bool doubleSided)
    {
        _doubleSided = doubleSided;
        _stateSetDirty = true;
    }

    bool AMaterialTemplate::removeParameter(const std::string& name)
    {
    ...
    void AMaterialTemplate::setSource(const std::string& source)
    {
        _source = source;
        _shaderDirty = true;
    }

    void AMaterialTemplate::apply()
    {
    ...
    int AMaterialTemplate::getAvailableUnit()
    {
    ...
    void AMaterialTemplate::applyParameters()
    {
        for (auto& parameter : _parameters)
        {
            switch (parameter.second.index())
            ...
            {
                TextureAndUnit& textureAndUnit = std::get<TextureAndUnit>(parameter.second);
                _stateSet->addUniform(new osg::Uniform(("u" + parameter.first).c_str(), textureAndUnit.second));
    ...
    std::string AMaterialTemplate::getParameterTypeString(const Parameter& parameter)
    {
        ...
        case size_t(ParameterIndex::Texture):
        {
            switch (std::get<TextureAndUnit>(parameter).first->_texture->getTextureTarget())
            {
            case GL_TEXTURE_2D:
                return "sampler2D";
    ...
    AMaterialInstance::AMaterialInstance() : AMaterial(Type::MaterialInstance) {}

    void AMaterialInstance::serialize(Json& json, std::vector<char>& binary, std::vector<std::string>& reference) const
    {
