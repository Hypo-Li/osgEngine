...
uniform int iRepeateNumber;
uniform float WaterColorBright;
uniform vec4 waterColor; 
uniform bool  BottomTextureState;        
in vec2 Coord;
in vec2 Coord1;
#define ENABLE_WATER
#define ENABLE_FOAM
...
{
    return vec2( cos(iFlowAngle), sin(iFlowAngle) ) * iFlowSpeed;
}
...
void RaymarchScene( vec3 vRayOrigin, vec3 vRayDir, out Intersection intersection )
{
    ...
    vec2 vGradVec = normalize( vec2( 1.0, 2.0 ) ); 
...
#else
    #if 0
    surface.m_albedo = texture( iChannel0, intersection.m_pos.xz * iRepeateNumber ).rgb;
    surface.m_albedo = surface.m_albedo * surface.m_albedo;
    #else
    vec3 vWeights = surface.m_normal * surface.m_normal;
    vec3 col = vec3(0.0);
    vec3 _sample;
    _sample = texture( iChannel0, intersection.m_pos.xz * iRepeateNumber ).rgb;
    col += _sample * _sample * vWeights.y;
    _sample = texture( iChannel0, intersection.m_pos.xy * iRepeateNumber ).rgb;
    col += _sample * _sample * vWeights.z;
    _sample = texture( iChannel0, intersection.m_pos.yz * iRepeateNumber ).rgb;
...
void BlockRender(in vec2 fragCoord)
{
    ...
    float frame = floor(osg_FrameTime * blockRate);
...
void riverEffectFS(inout vec4 color)
{
    g_fTime = osg_FrameTime;

#ifdef ENABLE_SCREENSHOT_MODE
    BlockRender( Coord.xy );
    float fBaseTime = k_screenshotTime;
#else
    float fBaseTime = osg_FrameTime;
#endif
    g_fTime = fBaseTime;
	...
	vec3 vCameraTarget = vec3(0.0, -1.3, 0.0);
    ...
    vCameraPos = vec3(0.085, 0.7, 0.9);
    vCameraTarget = vec3(0.0, -1.0, 0.0);
    ...
    for(int i=0; i<k_superSampleCount; i++)
    {
        ...
        vec3 vRandom = vec3( SmoothNoise( Coord.xy + fTot ), 
                        SmoothNoise( Coord.yx + fTot + 42.0 ),
                        SmoothNoise( Coord.xx + Coord.yy + fTot + 42.0 ) ) * 2.0 - 1.0;
    ...
    vFinal = (WaterColorBright * 0.1) * vFinal;
vFinal =vec3(vFinal.r*waterColor.r,vFinal.g*waterColor.g,vFinal.b*waterColor.b);
   vec4 riverBottomColor = texture(iChannel1, Coord1);
   float fWeight=0.0;    
  if(BottomTextureState) fWeight=1.0-waterColor.w;    
   color = mix(vec4(vFinal, 1.0), riverBottomColor, fWeight);
}
