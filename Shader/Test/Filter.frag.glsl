...
void main()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            float value = textureLod(uMaskTexture, uv + uResolution.zw * vec2(i - 1, j - 1), 0).r;
            edgeX += value * kernelX[i * 3 + j];
            edgeY += value * kernelY[i * 3 + j];
        }
    }
    float edge = saturate(sqrt(edgeX * edgeX + edgeY * edgeY));
    vec3 color = texelFetch(uCurrentColorTexture, ivec2(gl_FragCoord.xy), 0).rgb;
    if (edge != 0.0)
    {
        color = taa();
    }

    fragData = vec4(color, 1);
}