...
{
    vec2 uv = p.xy;
    vec3 weatherData = textureLod(uCloudMapTexture, uv, 0.0).rgb;
    return weatherData;
}

float sampleCloudDensity(vec3 p, vec3 weatherData, bool doCheap)
{
    const vec3 wind_direction = vec3(1.0, 0.0, 0.0);
    const float cloud_speed = 0.01;
    const float cloud_top_offset = 2.0;

    float normalizedHeight = saturate((length(p) - cloudBottomRadius) / cloudThickness);
...
float sampleCloudDensityAlongCone(vec3 p, float stepSize, float cosAngle, float densityAlongViewCone)
{
    vec3 lightStep = stepSize * uSunDirection;
    float coneSpreadMultplier = length(lightStep);
    ...
    for (int i = 0; i <= 6; ++i)
    {
        vec3 conePos = p + coneRadius * (coneSpreadMultplier * noiseKernel[i] * float(i));
        int mipOffset = int(i * 0.5);
        weatherData = sampleWeather(conePos);

        if (densityAlongViewCone < 0.3)
        {
            densityAlongCone += sampleCloudDensity(conePos, weatherData, false);
        }
        else
        {
            densityAlongCone += sampleCloudDensity(conePos, weatherData, true);
        }
...
vec4 rayMarchCloud(vec3 worldPos, vec3 worldDir)
{
    ...
    for (uint i = 0; i < cloudMarchingStepCount; ++i)
    {
        if (alpha <= 1.0)
        {
            ...
            if (cloudTest > 0.0)
            {
                float sampleDensity = sampleCloudDensity(samplePos, weatherData, false);
            ...
            {
                cloudTest = sampleCloudDensity(samplePos, weatherData, true);
                if (cloudTest == 0.0)
                {
                    sampleT += stepSize * 2;
                }
